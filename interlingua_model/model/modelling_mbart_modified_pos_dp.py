...
from .modeling_utils_mod_pos_dp import PreTrainedModel
...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        ### for token deprel embedding:
        self.embed_positions_deprel =  nn.Embedding(config.vocab_size, embed_dim, self.padding_idx)

        ### for token pp embedding:
        self.embed_positions_pp =  nn.Embedding(config.vocab_size, embed_dim, self.padding_idx)
    ...
    def forward(
        ...
        #     input_ids = input_ids
        # retrieve input_ids and inputs_embeds
        input_ids, pos, deprel, pp = input_ids
        if input_ids is not None and inputs_embeds is not None:
            raise ValueError("You cannot specify both input_ids and inputs_embeds at the same time")
        elif input_ids is not None:
            input_shape = input_ids.size()
        ...
        embed_deprel_tag = self.embed_positions_deprel(deprel) * self.pos_embed_scale
        embed_pp_tag = self.embed_positions_pp(pp) * self.pos_embed_scale
        #print("pos-tag-embed-generated")

        hidden_states = inputs_embeds + embed_pos + embed_pos_tag + embed_deprel_tag + embed_pp_tag######################################## here we can add the interlingua vector
        hidden_states = self.layernorm_embedding(hidden_states)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            """
            input_ids ---> model.generatate-->translated_string---> dep parser
            label_ids----> detokenize---> string---> dep parser
            
            
            
            """
            #print("model_output")
            self.check_output = {'input_ids':input_ids, 'lm_logits':lm_logits.view(-1, self.config.vocab_size), 'lables':labels,'transformed_labels':labels.view(-1)}
