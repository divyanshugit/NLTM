    ...
    def __call__(self, features, return_tensors=None):
        ...
        if labels is not None:
            ...
            for feature in features: # across each sentence: dictionary
                ...
                if "deprel" in feature.keys():
                                deprel_lis = feature["deprel"] # list converted to tensor
                                #pos_tensor_padded = F.pad(torch.Tensor(pos_tensor),pad=(0,max_len - len(pos_tensor)), mode='constant',value = 0)
                                feature_deprel.append(deprel_lis)
                                del feature["deprel"]  ###to delete pos
                if "pp" in feature.keys():
                                pp_lis = feature["pp"] # list converted to tensor
                                #pos_tensor_padded = F.pad(torch.Tensor(pos_tensor),pad=(0,max_len - len(pos_tensor)), mode='constant',value = 0)
                                feature_pp.append(pp_lis)
                                del feature["pp"]  ###to delete pos
        ...
        feature_deprel_padded = [F.pad(torch.Tensor(deprel).to(d_type_input_ids),pad=(0,input_ids_max_len - len(deprel)), mode='constant',value = 0) for deprel in feature_deprel ]
        #print("padding-done-pos")
        feature_deprel_tensor = torch.stack((feature_deprel_padded))
        feature_deprel_dict_final = {"deprel":feature_deprel_tensor}
        feature_pp_padded = [F.pad(torch.Tensor(pp).to(d_type_input_ids),pad=(0,input_ids_max_len - len(pp)), mode='constant',value = 0) for pp in feature_pp ]
        #print("padding-done-pos")
        feature_pp_tensor = torch.stack((feature_pp_padded))
        feature_pp_dict_final = {"pp":feature_pp_tensor}
        ...
        features["input_ids"] = (features["input_ids"], 
                                 feature_pos_dict_final["pos"],
                                 feature_deprel_dict_final["deprel"],
