...
from .modeling_utils_kd import PreTrainedModel
...
class MBartForConditionalGeneration(MBartPreTrainedModel):
    ...
    def forward(
        ...
        print("inside-mbartconditionalgen", len(input_ids))
        input_ids, hi_enc, ka_dec, input_ids_1, input_ids_2  = input_ids # comes batch wise
        #hi_enc = torch.tensor(i)
        self.enc_hi = hi_enc # list
        self.dec_ka = ka_dec # list
        self.input_ids_1 = input_ids_1 # list
        self.input_ids_2 = input_ids_2 # list
        self.org_input_ids = input_ids # list
        ...
        def get_hi_enc(sentence):
            self.model._modules["encoder"]._modules["layers"]._modules["5"]._modules["final_layer_norm"].register_forward_hook(get_activation('fc3'))
            output = self.model(sentence)
            return  activation['fc3']
        
        def get_ka_dec(sentence):
            self.model._modules["decoder"]._modules["layers"]._modules["5"]._modules["final_layer_norm"].register_forward_hook(get_activation('fc3'))
            output = self.model(sentence)
            return  activation['fc3']

        #enc_val = get_hi_enc(input_ids)
        #dec_val = get_ka_dec(input_ids)
        
        #hi_enc = torch.Tensor(hi_enc)
        #ka_dec = torch.Tensor(ka_dec)
        #print("types", type(hi_enc), type(ka_dec), type(enc_val), type(dec_val))
        #print("shapes", hi_enc.shape, ka_dec.shape, enc_val.shape, dec_val.shape)
        device = self.model.device
        loss_kd_fct = MSELoss()
        for i in range(len(input_ids)):
            #hi_enc[i][0]
            #ka_dec[i][0]
            enc_val = get_hi_enc(torch.tensor([input_ids_1[i]], device = device))
            dec_val = get_ka_dec(torch.tensor([input_ids_2[i]], device = device))
            loss_kd = loss_kd_fct(torch.tensor(hi_enc[i], device = device), enc_val)
            loss_kd += loss_kd_fct(torch.tensor(ka_dec[i], device = device), dec_val)
            #loss_batch_kd += loss_enc_kd + loss_dec_kd 
            ...
            #kd_loss = loss_kd_fct(enc_val, hi_enc) + loss_kd_fct(dec_val, ka_dec)
        ...
        if labels is not None:
            print(" reaching loss_fct")
            loss_fct = CrossEntropyLoss()
            masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
            print("loss_mlm",masked_lm_loss)
            
            
            kd_masked_lm_loss = masked_lm_loss + loss_kd
            print("combined_loss",kd_masked_lm_loss)
